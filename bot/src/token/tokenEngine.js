...
const { isConnected: isPgConnected, query, updateUserTokens, addTransaction, getTransactions } = require('../database/replitPg');
...
class TokenEngine {
  ...
  // ─── Set saldo langsung (untuk sync dari web DB saat login) ───
  setBalance(userId, balance) {
    if (!this.accounts.has(userId)) this.registerAccount(userId);
    const account = this.accounts.get(userId);
    account.balance = Math.max(0, Number(balance) || 0);
    console.log(`[DLAVIE][TOKEN] Balance synced for ${userId}: ${account.balance} tokens`);
    return account.balance;
  }
