...
// Hook webAuth ke ownerUtils agar isOwnerMsg bisa cek session role
const { setWebAuthGetter } = require('./utils/ownerUtils');
const { getWebAuth }       = require('./auth/webAuth');
setWebAuthGetter(getWebAuth);
...
async function connectToWhatsApp() {
  ...
  try {
    ...
    sock = makeWASocket({
      ...
      // macOS + Chrome: best fingerprint untuk render interactiveMessage di WA Business iOS
...
function scheduleReconnect(delayMs) {
  ...
  if (retryCount >= MAX_RETRIES) {
    console.warn(`[DLAVIE][WA] ⚠️ Gagal connect setelah ${MAX_RETRIES}x percobaan.`);
    console.warn('[DLAVIE][WA]    Keep-Alive watchdog akan coba reconnect dalam 10 menit...');
    // Reset retry counter setelah delay panjang — watchdog akan trigger forceReconnect
    reconnectTimer = setTimeout(() => {
      console.log('[DLAVIE][WA] 🔄 Auto-rescue: reset retry counter & reconnect...');
      retryCount   = 0;
      isConnecting = false;
      connectToWhatsApp().catch(e => {
        console.error('[DLAVIE][WA] Auto-rescue error:', e.message);
        isConnecting = false;
      });
    }, 10 * 60 * 1000);
    return;
...
// ─── Force reconnect (called by keepAlive watchdog) ───
function forceReconnect() {
  console.log('[DLAVIE][WA] 🔄 forceReconnect() called by watchdog');
  if (reconnectTimer) clearTimeout(reconnectTimer);
  retryCount   = 0;
  isConnecting = false;
  connectToWhatsApp().catch(e => {
    console.error('[DLAVIE][WA] forceReconnect error:', e.message);
    isConnecting = false;
  });
}
...
module.exports = { connectToWhatsApp, getSock, clearAuthState, forceReconnect, getConnectionState };
