...
module.exports = {
  ...
  execute: async (sock, msg, args, config, ctx = {}) => {
    ...
    // Owner tidak perlu login — cek WA number DAN session role
    if (isOwnerMsg(msg, config.ownerNumber)) {
      await safeSend(jid, {
        text: `👑 *Kamu adalah Owner DLavie OS!*\n\nKamu tidak perlu login. Semua command owner sudah aktif.\n\nGunakan \`!owner\` atau \`!status\` untuk info sistem.`
      });
      return;
    }
    ...
    // Verifikasi kode
    const result = await webAuth.verifyBotCode(userId, code);
