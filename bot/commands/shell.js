...
const { extractSenderNumber, isOwnerById } = require('../src/utils/ownerUtils');
function senderId(msg) { return extractSenderNumber(msg); }
...
module.exports = {
  ...
  execute: async (sock, msg, args, config, ctx = {}) => {
    ...
    // ─── Permission check ───
    const isOwner = isOwnerById(userId, config.ownerNumber || config.bot?.ownerNumber);
